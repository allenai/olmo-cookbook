...
@click.option("--output-dir", "-o", required=True, help="Output directory for merged models")
@click.option(
    "--methods",
    "-m",
    multiple=True,
...
@click.option(
    "--format",
    type=click.Choice(["safetensors", "pytorch", "auto"]),
    default="auto",
    show_default=True,
...
@click.option("--beaker-image", type=str, default="oe-eval-beaker/oe_eval_olmo3_auto", help="Beaker image to use")
...
def validate(checkpoint_path: str):
    ...
    has_model = (
        ...
        or len(glob.glob(str(path / "pytorch_model-*.bin"))) > 0
        or len(glob.glob(str(path / "model-*-of-*.safetensors"))) > 0
    )

    if not has_model:
        click.echo(f"No model files found in {checkpoint_path}")
        click.echo(
            "Expected one of: model.safetensors, pytorch_model.bin, pytorch_model-*.bin, model-*-of-*.safetensors"
        )
        return
