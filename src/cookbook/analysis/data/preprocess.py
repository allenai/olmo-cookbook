...
MODEL_OUTPUT_TO_KEEP = ["sum_logits", "logits_per_char", "logits_per_byte", "bits_per_byte"]

SIZE_PREFIXES = [
    f"-{size}-"
    for size in ["3B", "1B", "760M", "750M", "530M", "370M", "300M", "190M", "150M", "90M", "60M", "20M", "4M"]
]
SIZE_PREFIXES_FIX = {"3B": "3.2B", "1B": "1.3B"}

CHINHILLA_MULT = ["0.5xC", "1xC", "2xC", "5xC", "10xC", "15xC", "20xC"]
...
def compute_mean_safe(predictions, key):
    values = [pred[key] for pred in predictions if key in pred and pred[key] is not None]
...
def process_predictions(file_path):
    ...
    for pred in predictions:
        ...
        # TODO(undfined): This is not valid, see if we actually need it
        # if ("primary_score" not in entry and primary_metric_key in metrics) or (
        #     "primary_score" in entry and primary_metric_key["primary_score"] is None
        # ):
        if "primary_score" not in entry and primary_metric_key in metrics:
...
def process_metrics(file_path: str):
    """Process a metrics.json to a dict"""
    with open(file_path, "r") as f:
        results = json.load(f)

    if "beaker_info" in results:
        del results["beaker_info"]
    if "compute_config" in results:
        del results["compute_config"]
    if "task_config" in results:
        del results["task_config"]
    ...
    if "metrics" in results:
        for metric in results["metrics"]:
            if (
                "paloma" in file_path or "llm_compression" in file_path or "custom_loss" in file_path
            ) and metric not in PALOMA_METRICS:
                continue
            ...
            for prediction in predictions:
                if "correct_choice" in prediction and prediction["correct_choice"] is not None:
                    try:
                        ...
                        if (
                            "logits_per_byte_corr" not in prediction or prediction["logits_per_byte_corr"] is None
                        ) and "logits_per_byte" in prediction:
                        ...
                        if (
                            "logits_per_char_corr" not in prediction or prediction["logits_per_char_corr"] is None
                        ) and "logits_per_char" in prediction:
...
def load_df_parallel(data, file_type: str, usage_threshold: int = 80):
    ...
    with Pool(processes=len(available_cpus)) as pool:
        dataframes = list(
            tqdm(pool.imap(process_chunk, chunks), desc="Converting to Pandas dataframe", total=len(chunks))
        )
...
def load_file(file_data, _type):
    ...
    if _type == "predictions":
        ...
        raise ValueError(f"Unknown file type: {_type}")
    ...
    with tqdm(desc="Scanning paths", total=len(paths), unit="path") as pbar:
        for path in paths:
            if os.path.isfile(path):
                if str(path).endswith("-predictions.jsonl") or str(path).endswith("-metrics.json"):
