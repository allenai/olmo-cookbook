...
ENLARGE_TASKS_OLMES = [
    task.replace(":rc", ":enlarge") for task in RC_TASKS_OLMES if "winogrande" not in task and "mmlu" not in task
]
DISTRACTORS_TASKS_OLMES = [
    task.replace(":rc", ":distractors")
    for task in RC_TASKS_OLMES
    if "winogrande" not in task and "mmlu" not in task
]
...
CUSTOM_LOSS = ["sky_t1::custom_loss", "numia_math::custom_loss", "tulu_if::custom_loss"]
...
AGI_EVAL_COT = [
    task.replace("::olmes:full", ":cot::none") for task in AGI_EVAL_MC
