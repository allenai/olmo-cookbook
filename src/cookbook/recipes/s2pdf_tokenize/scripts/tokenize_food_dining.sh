...
output_dir="/mnt/raid0/allenai/dolma2-tokenizer/food_dining" 

mkdir -p "$output_dir"

uv run dolma tokens \
    --documents "$src_path/food_dining/*.zst" \
...
s5cmd cp -sp "$output_dir/" s3://ai2-llm/preprocessed/s2pdf_dedupe_minhash_v1_with_no_pii_basic_quality_datadelve_norefs_mdtables_v2_denylisted_reshard_denyagain_compressionv2/food_dining/allenai/dolma2-tokenizer/