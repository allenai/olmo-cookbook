...
def main(token_count: int, min_vigintile: int, npy_prefix: str, output: str) -> None:
    ...
    topic_vig_hier = {}
    for key, size in npy_prefix_sizes.items():
    ...
    with tqdm.tqdm(total=token_count, desc="Selecting files", unit_scale=True) as pbar:
        for topic, vigintiles in topic_vig_hier.items():
            for vigintile, files in vigintiles.items():
                current_tokens = sum(files.values()) // 4
                desired_file_total = sampling_ratio * current_tokens
                ...
                for file_path, size in current_files:
                    file_tokens = size // 4
                    if current_size + (file_tokens // 2) > desired_file_total:
                        break

                    output_obj["dataset"]["sources"][0]["paths"].append(file_path)
                    current_size += file_tokens
                    pbar.update(file_tokens)

                print(
                    f"{topic}/{vigintile}: "
                    f"requested {desired_file_total / 1024 ** 3:.3f} B; "
                    f"selected {current_size / 1024 ** 3:.3f} B; "
                    f"error {(current_size - desired_file_total) / desired_file_total:.2%}"
                )

    output_obj["dataset"]["sources"][0]["paths"] = sorted(output_obj["dataset"]["sources"][0]["paths"])
