...
while IFS= read -r line; do
    ...
    # Run the s5cmd copy command - note the trailing slash on destination
    s5cmd cp -sp "${path}${tokenizer}/" "s3://ai2-llm/preprocessed/${path_to_data}/dolma2-tokenizer/"
    
    # Check if the command was successful
    if [ $? -eq 0 ]; then
        echo "Successfully copied: ${path}${tokenizer}/ -> s3://ai2-llm/preprocessed/${path_to_data}/dolma2-tokenizer/"
    else
        echo "Error copying: ${path}${tokenizer}/"
    fi
